	...
	<body>
	    ...
	    <form action="signup_check" id="signup-form" method="post">
	        <div class="gc004-container">
	            ...
	            <hr><label class="form-label fw-bold" for="id">ID</label><span id="member_id-span"></span><input type="text" placeholder="Enter ID" name="id" required="">
	            <label class="form-label fw-bold" for="psw">Password</label><input type="password" placeholder="Enter Password" name="pass" required="">
	            <label class="form-label fw-bold" for="psw">Repeat Password</label><input type="password" placeholder="Repeat Password" name="pass-repeat" required="">
		...
		$("#signup-form").onsubmit=function(){
		    let checkfails=function(object){
		        ...
		        swal({
		            ...
		            object.value="";
		            object.focus();
		            if(object.name=="pass-repeat")
		            	$("input[name=pass]").focus();
		        });
		    }
		    if(!(reg_id.test($("input[name=id]").val()))||checked_id=='false'){
		    ...
		    if(!($("input[name=pass]").val()==$("input[name=pass-repeat]").val())){
		    ...
		    if($("input[name=build]").val()==""){
		        checkfails($("input[name=build]"));
		        return false;
		    }
		}
		...
		$("input[name=id]").focusout(function(){
		  	if(reg_id.test($("input[name=id]").val())){
		  		if(checked_id){
		  			...
		  			$.post("id_check",{id:$("input[name=id]").val()},function(data){
		  				if(data=="true"){

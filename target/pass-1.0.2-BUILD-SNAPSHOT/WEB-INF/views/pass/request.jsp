...
<!-- 		        </form> -->
		...
		<script>
			...
			$("#submit-btn").on("click",function(){
				...
				if($("#service_description tbody tr").length>0&&$("#request_date").val()!=null){
					let list=new Array();
					for(let i=0;i<$("#service_description tbody tr").length;i++){
						list.push({"ordercost_no":$("#service_description tbody").find("tr:eq("+i+")").attr("id"),"amount":$("#service_description tbody").find("tr:eq("+i+")").find("td:eq(2)").find("input").val()})
					}
					$.post("insertpass",{"request_date":date_str, "payment_check": $(".form-check-input:checked").val(),"address":$("#address").val(),"OrderList":list});
				}
				else{
			        swal({
			            title : "입력 오류",
			            icon : "error",
			            text : "수거 물품을 하나이상 등록해주세요",
...
// 				for(let i=0;i<$("#service_description tbody tr").length;i++){
// 					alert($("#service_description tbody").find("tr:eq("+i+")").attr("id"));
// 					alert($("#service_description tbody").find("tr:eq("+i+")").find("td:eq(1)").html());
// 					alert($("#service_description tbody").find("tr:eq("+i+")").find("td:eq(2)").find("input").val());
// 				}
// 				return false;
			})
		</script>
